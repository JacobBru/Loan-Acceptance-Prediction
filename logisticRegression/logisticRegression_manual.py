...
def Cost_Function_Derivative(X, Y, theta, j, m, alpha):
    sumErrors = 0
    for i in range(m):
        xi = X.iloc[i]  # Fixed row access
        xij = xi[j]
        hi = Prediction(theta, xi)
        error = (hi - Y.iloc[i]) * xij
        sumErrors += error
...
theta = np.zeros(X.shape[1])  # Adjusted for the correct number of features
...
for x in range(max_iteration):
    ...
    X_batch = trainX.iloc[indices]
    Y_batch = trainY.iloc[indices]
